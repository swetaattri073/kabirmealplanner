...
| `ADMIN_EMAILS` | Comma-separated emails allowed to open `/admin` | empty (nobody) |
...
Auth is email/password only (social login removed). Chat is Premium-only and off until `FEATURE_CHAT_ENABLED=true`. Set `ADMIN_EMAILS=you@example.com` to unlock the operator dashboard at `/admin` (also linked from Account for those emails).
